...
export default function App() {
    useEffect(() => {
        ...
        db.transaction(function (txn) {
            ...
            txn.executeSql("CREATE TABLE IF NOT EXISTS table_orders(order_id INTEGER PRIMARY KEY AUTOINCREMENT, user_id VARCHAR(64), order_date DATETIME, total_amount INTEGER )", []);
        });
        db.transaction(function (txn) {
            console.log('order tablosu oluşturuluor');
            txn.executeSql("CREATE TABLE IF NOT EXISTS order_items(order_item_id INTEGER PRIMARY KEY AUTOINCREMENT, order_id INTEGER, product_id INTEGER, quantity INTEGER NULL )", []);
        });
    }, []);
